...
const scope = ['routes/sanwatergroup', 'components/dashboard', 'components/products/ProductForm.jsx'];
const namespaces = [
...
for (const file of scope.flatMap((p) => files(path.join(root, p))).filter((p) => /\.jsx$/.test(p))) {
  ...
  traverse(ast, {
    StringLiteral(p) {
      ...
      const call = p.findParent((parent) => parent.isCallExpression());
      if (!call) return;
      const callee = call.node.callee;
      const isToast = callee.type === 'MemberExpression' && callee.object.name === 'toast' && ['error', 'success', 'warning', 'info'].includes(callee.property.name);
      const isError = callee.type === 'Identifier' && ['setError', 'setSuccess', 'setMessage', 'confirm', 'alert'].includes(callee.name);
      const isConfirm = callee.type === 'MemberExpression' && callee.object.name === 'window' && ['confirm', 'alert'].includes(callee.property.name);
      if (!isToast && !isError && !isConfirm) return;
...
console.log(`Migrated ${count} message literals`);
