...
export default function ArticleContent({ article, preview = false }) {
  ...
  return <article className="overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-sm">
    {article?.coverImage && <img src={article.coverImage} alt={article.title || t("admin.news.article_cover")} className="max-h-[520px] w-full object-cover" />}
    <div className="p-6 sm:p-10 lg:p-12">
      <div className="flex flex-wrap items-center gap-3 text-xs font-semibold text-slate-500">
        ...
        <span className="inline-flex items-center gap-1.5"><CalendarDays className="h-4 w-4" />{displayDate ? new Date(displayDate).toLocaleDateString(lang) : t("admin.news.unscheduled")}</span>
        {article?.author && <span>{t("admin.news.by_author", { author: article.author })}</span>}
      </div>
    ...
    {products.length > 0 && <section className="border-t border-slate-200 bg-slate-50 p-6 sm:p-10">
      <div className="flex items-center gap-2"><Package className="h-5 w-5 text-blue-600" /><h2 className="text-xl font-bold">{t("admin.news.related_products")}</h2></div>
    ...
    <section className="m-6 rounded-3xl bg-blue-600 p-6 text-white sm:m-10 sm:flex sm:items-center sm:justify-between">
      <div><h2 className="text-xl font-bold">{t("admin.news.help_choosing")}</h2><p className="mt-1 text-sm text-blue-100">{t("admin.news.sales_help_description")}</p></div>
      <Link to={CONTACTSALES} onClick={() => !preview && trackCustomEvent("article_contact_clicked", { article_id: article._id, article_slug: article.slug, page: window.location.pathname })} className="mt-4 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-bold text-blue-700 sm:mt-0">{t("admin.news.contact_sales")} <ArrowRight className="h-4 w-4" /></Link>
    </section>
  </article>;
}
